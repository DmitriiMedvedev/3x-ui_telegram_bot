...
def get_user_by_sub(sub_id: str) -> dict | None:
    ...
    try:
        ...
        if sub_id.isdigit():
            row = conn.execute(
                "SELECT * FROM users WHERE tg_id=?", (int(sub_id),)
            ).fetchone()
...
async def fetch_panel_sub(
    session: aiohttp.ClientSession, panel: dict, sub_id: str
) -> str:
                ...
                for sp in sub_paths:
                    ...
                    try:
                        async with session.get(
                            url, headers=headers, timeout=3.0
                        ) as resp:
                            if resp.status == 200:
                                text = await resp.text()
                                if text and len(text) > 10:
                                    logger.info(
                                        f"Success fetching sub from {panel['name']} via {url}"
                                    )
                                    return text
                            else:
                                logger.debug(
                                    f"Sub fetch 404/Error from {panel['name']} via {url}: {resp.status}"
                                )
                    except Exception as e:
                        logger.debug(
                            f"Sub fetch exception from {panel['name']} via {url}: {e}"
                        )
    return ""
...
def make_fallback_link(u_uuid, email, panel, cfg, iid):
    ...
    try:
        ...
        host = (
            cfg.get("host")
            or panel.get("server_host")
            or panel.get("host")
            or "127.0.0.1"
        )
        port = cfg.get("port", 443)
        label = urllib.parse.quote(
            f"{email}-{panel.get('name')}-{cfg.get('label', iid)}"
        )
        ...
        if prot == "vless":
            ...
            if net == "xhttp":
                ...
                params.update(
                    {"serviceName": cfg.get("grpc_service", ""), "mode": "gun"}
                )
            elif net == "ws":
...
async def handle_sub(request: web.Request) -> web.Response:
    try:
        ...
        if not user:
            return web.Response(
                text=f"ERROR: User with Sub ID '{sub_id}' not found in Database.",
                status=404,
            )
        ...
        diag_log = [
            f"Sub Request: {sub_id}",
            f"User: {email}",
            f"UUID: {u_uuid}",
            f"Panels count: {len(panels)}",
        ]
        ...
        async with aiohttp.ClientSession(headers=HEADERS) as session:
            ...
            for i, res in enumerate(results):
                ...
                if res:
                    ...
                    diag_log.append(
                        f" - Panel '{p['name']}': Success, got {len(links)} links."
                    )
                else:
                    diag_log.append(
                        f" - Panel '{p['name']}': Failed (unreachable or 404)."
                    )

        # 2. Если проксирование не дало результатов, генерируем сами (Fallback mode)
        if not all_links:
            diag_log.append(
                "Proxy failed for all panels. Trying fallback generation..."
            )
            for p in panels:
                ...
                for iid, cfg in ibs.items():
                    ...
                    if link:
                        ...
                        p_links += 1
                diag_log.append(
                    f" - Fallback '{p['name']}': generated {p_links} links."
                )
...
async def handle_index(request):
    return web.Response(
        text=f"Dobrinya Subscription Server v17.1 is active.\nDB: {'✅' if os.path.exists(DB_PATH) else '❌ NOT FOUND'}"
    )
