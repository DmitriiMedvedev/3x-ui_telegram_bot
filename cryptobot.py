...
async def _get(method: str, params: dict | None = None) -> dict | None:
    try:
        async with aiohttp.ClientSession() as s:
            async with s.get(
                f"{_BASE}/{method}", headers=_HEADERS, params=params
            ) as r:
                text = await r.text()
                if r.status != 200:
                    logger.error(f"CryptoBot GET {method} HTTP {r.status}: {text[:200]}")
                    return None
                import json
...
async def _post(method: str, payload: dict) -> dict | None:
    try:
        async with aiohttp.ClientSession() as s:
            async with s.post(
                f"{_BASE}/{method}", headers=_HEADERS, json=payload
            ) as r:
                text = await r.text()
                if r.status != 200:
                    logger.error(f"CryptoBot POST {method} HTTP {r.status}: {text[:200]}")
                    return None
                import json
                data = json.loads(text)
                if not data.get("ok"):
...
async def create_invoice(amount_rub: float, payload: str) -> dict | None:
    ...
    """
    result = await _post("createInvoice", {
        "currency_type": "fiat",
        "fiat":          "RUB",
